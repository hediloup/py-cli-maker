...
def modifier_{app_name.lower()}({app_name.lower()}_id: int, **kwargs) -> Optional[{model_name}]:
    """
...
def obtenir_{app_name.lower()}_par_id({app_name.lower()}_id: int) -> Optional[{model_name}]:
    """
...
<body>
    ...
    <ul>
        ...
        <li>
            <a href="{{% url '{app_name}:detail' {app_name}.pk %}}">{{{{ {app_name} }}}}</a>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{% if object %}}Modifier{{% else %}}Créer{{% endif %}} un {model_name}</title>
</head>
<body>
